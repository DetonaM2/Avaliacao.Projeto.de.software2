...
def grava_em_arquivo(linhas, arquivo, modo='w'):
    ...
    '''
    try:
        with open(arquivo, modo, encoding='utf-8') as file:
            file.write(linhas)
        file.close()
    except IOError:
        print("Erro ao abrir o arquivo {arquivo}")
...
def carrega_dicionario(arquivo_dicionario):
    ...
    '''
    input = le_linhas_arquivo(arquivo_dicionario)
    output = []
    for item in input:
        item = item.replace("\n", "")
...
def conta_mais_frequente(colecao):
    ...
    output = ["", 0]
    for item in colecao:
        if colecao.count(item) > output[1]:
            output = [item, colecao.count(item)]
    return output
...
def busca_sugestoes_correcao(palavras_oov, vocabulario):
    ...
    """
    output = []
    for palavra in palavras_oov:
        temp = []
        for dic in vocabulario:
            if len(dic) == len(palavra):
                quant = 0
                for i in range(len(dic)):
                    if dic[i] == palavra[i]:
                        quant += 1
                if quant:
                    temp.append((dic, quant))
        temp.sort(key=lambda x: x[1])
        output.append((palavra, temp[-1][0]))
    return output
